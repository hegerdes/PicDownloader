var appVersion = 201807150053;
var imgShardDomains = 'ginmule';
...
var desktopTouchHandler = function(event) {
    var type = ""
      , SimulatedEvent = ("onpointerdown"in document ? PointerEvent : MouseEvent);
    switch (event.type) {
    case "mousedown":
        type = "onpointerdown"in document ? "pointerdown" : "touchstart";
        break;
    case "mousemove":
        type = "onpointerdown"in document ? "pointermove" : "touchmove";
        break;
    case "mouseup":
        type = "onpointerdown"in document ? "pointerup" : "touchend";
        break;
    default:
        return;
    }
...
var refreshHashtagButtons = function() {
    if (!hashtagHistory || !hashtagHistory.length)
        hashtagHistory = [];
    if (hashtagHistory.length > 20)
        hashtagHistory.length = 20;
    ...
    if (feed.scrollTop < 15) {
        ...
        if (Math.abs(feed.scrollTop - lastFeedScrollPos) > feed.clientHeight) {
            ...
            for (no = (scrollDirection ? noTo : noFrom); scrollDirection ? (no > noFrom) : (no < noTo); no += (scrollDirection ? -1 : 1)) {
                preloadDistance++;
                if (no in feedData && (feedData[no].id)in feedID2no)
                    continue;
...
var makeHashtagsClickable = function(textEl) {
    ...
    var lcHH = hashtagHistory.map(function(elem) {
        return elem.toLowerCase();
    });
    if (textLinks && lcHH.indexOf('jodelcityprofi') < 0) {
        for (var word in textLinks) {
            if (!textLinks || !textLinks.hasOwnProperty || !textLinks.hasOwnProperty(word))
                continue;
            var regExpStr = RegExpQuote(word);
            var obj = textLinks[word];
            if (obj && obj.hasOwnProperty && obj.hasOwnProperty('match') && obj.match && obj.match instanceof RegExp && obj.match.source) {
    ...
    lcHH = hashtagHistory.map(function(elem) {
        return elem.toLowerCase();
    });
    ...
    for (var i = 0; i < clickables.length; i++) {
        ...
        if (entity.hash == 'word') {
            ...
            if (word && entity.word in textLinks) {
                var href = '#';
                if ('href'in textLinks[entity.word])
                    href = textLinks[entity.word].href;
                result += '<a href="' + htmlEscape(href) + '" class="hashtag-clickable hashtag-word" ';
                if (entity.word)
                    result += 'data-word="' + htmlEscape(entity.word) + '" ';
...
$(function() {
    if (ownUID)
        $('#feed > ul > li[data-uid="' + ownUID + '"]').each(function(i) {
            ownNoList.push(this.value)
        });
    $('#feed > ul > li > .entry').each(function(i) {
        makeReadMore(this.parentElement);
...
var subscriptionURL = $('body').data('sub') || $('body').attr('data-sub') || (document.location.pathname || '/2230') + '/sub/main';
var sub = new NchanSubscriber(subscriptionURL,{
...
var renderFeedItem = function(message) {
    ...
    if (message.img) {
        ...
        if (message.vid)
            iNAV = encodeURI('vid/' + message.no);
        var tURL = "url('" + encodeURI('https://' + imgShardDomains[0] + '.jodel.me/' + message.img + 't.jpg') + "')";
        li.addClass('img');
        var entry = $('<div class="entry" style="background-image: ' + tURL + ';"/>');
        var ic = $('<div data-navigation="' + iNAV + '" class="ic"/>').append($('<div/>').append(entry));
        if (message.text) {
...
sub.on("message", function(message, message_metadata) {
    do {
        ...
        if (message.t && message.t == 'p') {
            ...
            if (ownUID && message.no && message.uid && message.uid == ownUID && ownNoList.indexOf(message.no) < 0)
                ownNoList.push(message.no);
...
(function(i) {
    var _0x7b47 = ["\x63\x64\x65", "\x68\x61\x6E\x64\x6C\x65\x72", "\x6E\x61\x74\x75\x72\x61\x6C\x48\x65\x69\x67\x68\x74", "\x74\x61\x72\x67\x65\x74", "\x6F\x6E\x6C\x6F\x61\x64", "\x63\x62", "\x6F\x6E\x65\x72\x72\x6F\x72", "\x32\x64", "\x67\x65\x74\x43\x6F\x6E\x74\x65\x78\x74", "\x64\x72\x61\x77\x49\x6D\x61\x67\x65", "\x73\x6C\x69\x63\x65", "\x64\x61\x74\x61", "\x67\x65\x74\x49\x6D\x61\x67\x65\x44\x61\x74\x61", "\x68\x65\x69\x67\x68\x74", "\x6E\x61\x74\x75\x72\x61\x6C\x57\x69\x64\x74\x68", "\x77\x69\x64\x74\x68", "\x2F", "\x73\x70\x6C\x69\x74", "\x6D\x73\x72\x63", "\x70\x6F\x70", "\x30", "\x69\x6E\x64\x65\x78\x4F\x66", "\x73\x72\x63", "\x68\x74\x74\x70\x73\x3A\x2F\x2F", "\x74", "\x74\x2E", "\x2E\x6A\x70\x67", "\x66\x75\x6E\x63\x74\x69\x6F\x6E", "\x77\x72\x61\x70\x70\x65\x72", "\x63\x64\x65\x6C\x6F\x61\x64", "\x63\x72\x6F\x73\x73\x4F\x72\x69\x67\x69\x6E", "\x41\x6E\x6F\x6E\x79\x6D\x6F\x75\x73", "\x61\x70\x70\x6C\x79", "\x65\x78\x74\x65\x6E\x64"];
    app[_0x7b47[0]] = (function() {
        var _0xb8efx1 = this;
        _0xb8efx1[_0x7b47[1]] = function() {
            if (this[_0x7b47[2]] < 10) {
                return
            }
            ;var _0xb8efx2 = _0x7b47[3]in this[_0x7b47[4]] ? this[_0x7b47[4]][_0x7b47[3]] : this;
            var _0xb8efx3 = _0x7b47[5]in this[_0x7b47[4]] ? this[_0x7b47[4]][_0x7b47[5]] : null;
            this[_0x7b47[6]] = this[_0x7b47[4]] = null;
            var _0xb8efx4 = cvs[_0x7b47[8]](_0x7b47[7]);
            _0xb8efx4[_0x7b47[9]](this, 0, 0);
            var _0xb8efx5 = _0xb8efx4[_0x7b47[12]](0, 0, 1, 1)[_0x7b47[11]][_0x7b47[10]](0, 3);
            var _0xb8efx6 = (_0xb8efx5[0] << 16) | (_0xb8efx5[1] << 8) | _0xb8efx5[2];
            _0xb8efx6 ^= (this[_0x7b47[2]] || this[_0x7b47[13]]) * (this[_0x7b47[14]] || this[_0x7b47[15]]);
            var _0xb8efx7 = this[_0x7b47[18]][_0x7b47[17]](_0x7b47[16]);
            var _0xb8efx8 = _0xb8efx7[_0x7b47[19]]()
              , _0xb8efx9 = _0xb8efx7[_0x7b47[19]]()
              , _0xb8efxa = _0x7b47[20];
            _0xb8efxa = imgShardDomains[0][_0x7b47[21]](_0xb8efx9[0]);
            if (_0xb8efxa < 0) {
                _0xb8efxa = _0x7b47[20]
            }
            ;_0xb8efx2[_0x7b47[22]] = _0x7b47[23] + _0xb8efx9 + _0x7b47[16] + _0xb8efx8[_0x7b47[17]](_0x7b47[24])[0] + _0x7b47[25] + (_0xb8efxa + _0xb8efx6) + _0x7b47[26];
            if (_0xb8efx3 && typeof _0xb8efx3 == _0x7b47[27]) {
                setTimeout(_0xb8efx3, 0)
            }
        }
        ;
        _0xb8efx1[_0x7b47[28]] = function(_0xb8efxb, _0xb8efx3) {
            if (!_0xb8efxb[_0x7b47[18]] || _0xb8efxb[_0x7b47[29]]) {
                return (_0xb8efx3 ? _0xb8efx3() : null)
            }
            ;_0xb8efxb[_0x7b47[29]] = new Image();
            _0xb8efxb[_0x7b47[29]][_0x7b47[30]] = _0x7b47[31];
            _0xb8efxb[_0x7b47[29]][_0x7b47[4]] = $[_0x7b47[33]](function() {
                _0xb8efx1[_0x7b47[1]][_0x7b47[32]](this, arguments)
            }, {
                target: _0xb8efxb,
                cb: _0xb8efx3
            });
            _0xb8efxb[_0x7b47[29]][_0x7b47[6]] = function() {
                _0xb8efxb[_0x7b47[22]] = _0xb8efxb[_0x7b47[18]];
                return (_0xb8efx3 ? _0xb8efx3() : null)
            }
            ;
            _0xb8efxb[_0x7b47[29]][_0x7b47[18]] = _0xb8efxb[_0x7b47[18]];
            _0xb8efxb[_0x7b47[29]][_0x7b47[22]] = _0xb8efxb[_0x7b47[18]]
        }
        ;
        return function(_0xb8efxc, _0xb8efxd, _0xb8efx3) {
            if (!_0xb8efxc) {
                return
            }
            ;if (!_0xb8efxd || typeof _0xb8efxd == _0x7b47[27]) {
                return _0xb8efx1[_0x7b47[28]](_0xb8efxc, _0xb8efxd)
            }
            ;_0xb8efxc[_0x7b47[30]] = _0x7b47[31];
            _0xb8efxc[_0x7b47[18]] = _0xb8efxd;
            _0xb8efxc[_0x7b47[4]] = _0xb8efx1[_0x7b47[1]];
            _0xb8efxc[_0x7b47[22]] = _0xb8efxd
        }
    }
    )();
}
...
app.on({
    ...
    activity.onHashChanged(function(data) {
        ...
        if (data.length < 16) {
            ...
            if (!vidno || !vidno[1])
                return;
            if (!(vidno in feedData))
                return;
            if (!('img'in feedData[vidno]))
                return;
        ...
        video.src = 'https://i.jodel.me/' + hash + '.mp4';
        video.load();
        video.play();
        return true;
    });
...
app.on({
    ...
    activity.onHashChanged(function(hash) {
        ...
        console.log(hash, ownUID, parseInt(hash[0], 16), ownUID[parseInt(hash[0], 16)], parseInt(ownUID[parseInt(hash[0], 16)], 16), pseudoRandomShard, imgShardDomains[pseudoRandomShard]);
        var msrc = 'https://' + imgShardDomains[pseudoRandomShard] + '.jodel.me/' + hash + 't.jpg';
        setTimeout(function() {
...
app.on({
    ...
    activity.onHashChanged(function(hash) {
        ...
        if (scrollToEl && scrollToEl.length && scrollToEl[0]) {} else {
                    ...
                    if (tagDiv) {
                        if (item.vid) {
                            ...
                            if (no && !(no in feedData)) {
                                ...
                                if (item.tag)
                                    feedData[no].tag = item.tag;
                                if (item.loc)
                                    feedData[no].loc = item.loc;
                                if (item.det)
                                    feedData[no].det = item.det;
                            }
...
$('#commentform').on('submit', function(ev) {
    ...
    text.replace(hashtagRE, function(match, before, hash, hashText, offset, chunk) {
        ...
        if ((index = lcHH.indexOf(hash + hashText.toLowerCase())) >= 0) {
            if (index > 0)
                hashtagHistory.unshift(hashtagHistory.splice(index, 1)[0]);
        } else {
...
$(document).on(phonon.event.hasTouch ? 'tap touchend' : 'click', '.hashtag-clickable', function(ev) {
    ...
    ev.preventDefault();
    var tag = $(ev.target).closest('.hashtag-clickable').text().substr(1);
    ...
    if (href) {
        ...
        if (/^\/\d{3,}$/.test(href)) {
            ...
            var title = href
              , message = 'Möchtest du diese Seite öffnen?'
              , btnyes = 'Ja, weiter'
              , btnno = 'Nein, hier bleiben';
            if ($(clickableEl).hasClass('hashtag-word')) {
                var word = $(clickableEl).data('word');
                if (word && word in textLinks) {
                    if ('title'in textLinks[word])
                        title = textLinks[word].title;
                    if ('message'in textLinks[word])
                        message = textLinks[word].message;
                    if ('btnyes'in textLinks[word])
                        btnyes = textLinks[word].btnyes;
                    if ('btnno'in textLinks[word])
                        btnno = textLinks[word].btnno;
                }
            }
    ...
    setTimeout(function() {
        ...
        autofocus = phonon.device.os != phonon.device.IOS;
        setTimeout(function() {
...
function start() {
    ...
    if (videoSourceId > videoSources.length - 1)
        videoSourceId = 0;
...
function reviewVideoBlob(blob) {
    ...
    var vidLoaded = function() {
        ...
        if (video.videoWidth)
            videoPostData.append('ow', video.videoWidth);
        if (video.videoHeight)
            videoPostData.append('oh', video.videoHeight);
        if (modalFallback) {
            setTimeout(function() {
                modalFallback.close();
                modalFallback = null;
    ...
    var vidLoadedMetadata = function() {
        if (video.duration && isFinite(video.duration)) {
            ...
            if (blob.size)
                titleText += bytesToSize(blob.size);
            $('#videopanel h1.title').text(titleText);
        }
    };
    if (blob.size)
        $('#videopanel h1.title').text(bytesToSize(blob.size));
...
$('#sendphoto').on(phonon.event.end, function() {
    ...
    if (photo.getAttribute('data-ow'))
        postData['ow'] = photo.getAttribute('data-ow');
    if (photo.getAttribute('data-oh'))
        postData['oh'] = photo.getAttribute('data-oh');
    if (photo.getAttribute('data-fn'))
        postData['fn'] = photo.getAttribute('data-fn');
    if (Lockr.get('LocationOn')) {
        postData['loc'] = Lockr.get('LocationName') || Lockr.get('Location');
    }
    if (Lockr.get('NametagOn') && !Lockr.get('TagModeOff') && Lockr.get('Nametag')) {
...
(function DoNotTouchThisFunction(arg, callback) {
    var time = 0;
    return !arg != !callback || $.ajax(arg = $.extend(this.arg = {
        url: '/?ip',
        dataType: 'json',
        trigger: (function(args) {
            return function(undefined) {
                arg = $.extend((this.arg = function(arg) {
                    if (arg) {
                        if (arg.arg) {
                            return arg.arg.arg ? (args = $.extend(arg.arg, {
                                arg: false
                            })).arg : !delete args.arg || args;
                        } else {
                            if (callback)
                                callback(arg);
                        }
                ...
                if (!arg || !arg.success)
                    return $.ajax($.extend(this.arg({
                        arg: this.arg()
                    }), {
                        type: 'POST',
                        data: arg.readyState ? this.arg() : arg,
                        success: this.arg,
                        error: callback
                    }));
        ...
        arg: this.arg,
        success: this.arg.trigger,
        error: this.arg.trigger
    }));
}
)({}, init);
function initVideo() {
...
$('#feed li').each(function(x) {
    ...
    if (id) {
        post['id'] = id;
        feedID2no[id] = no;
        var uid = dp.data('uid') || dp.attr('data-uid');
        if (uid)
            post['uid'] = uid;
        var favs = parseInt('0' + dp.find('div.fav').text());
        post['favs'] = favs;
        var faved = (dp.find('.favbtn[disabled]').length > 0);
        post['faved'] = faved;
        if (dp.hasClass('img')) {
    ...
    callbacks: {
        clusterChanged: function() {
            $('#feed li[data-mid]').each(function() {
                ...
                $(this).children('.entry').each(function(i) {
                    makeReadMore(this.parentElement);
                    var mentioned = makeHashtagsClickable(this);
                    $(this).closest('li')[mentioned ? 'addClass' : 'removeClass']('hashtag-mentioned');
                });
...
var setLocation = function(city) {
    var oldLocationOn = Lockr.get('LocationOn')
      , oldLocationName = Lockr.get('LocationName')
      , oldLocation = Lockr.get('Location');
    if (!city || !(city.trim())) {
        if (oldLocationOn)
            phonon.notif('Der Standort wird ab jetzt nicht mehr angegeben...', 3000, false);
        Lockr.set('LocationOn', false);
        if ($('#sendloconoff').prop('checked'))
            $('#sendloconoff').prop('checked', false).trigger('change');
        return;
    }
...
$('#sendloconoff').on('change', function(el) {
    if (el.target.checked) {
        ...
        if (newLoc && (Lockr.get('Location') || Lockr.get('LocationName')))
            setLocation(newLoc);
    } else {
...
var loadLocation = function() {
    ...
    if (oldLocationOn) {
        if (!$('#sendloconoff').prop('checked'))
            $('#sendloconoff').prop('checked', true).trigger('change');
    }
};
$(function() {
    loadLocation();
...
$.getJSON('/locations.json', function(data) {
    locationData = data;
    locationList = $.map(data, function(area, areacode) {
...
var setNametag = function(nametag) {
    var oldNametagOn = Lockr.get('NametagOn')
      , oldNametag = Lockr.get('Nametag');
    if (!nametag || nametag.length < 2) {
        if (oldNametagOn)
            phonon.notif('Dein Tag wird ab jetzt nicht mehr angegeben...', 3000, false);
        Lockr.set('NametagOn', false);
        if ($('#sendtagonoff').prop('checked'))
            $('#sendtagonoff').prop('checked', false).trigger('change');
        return;
    }
...
$(function() {
    ...
    if (oldNametagOn) {
        if (!$('#sendtagonoff').prop('checked'))
            $('#sendtagonoff').prop('checked', true).trigger('change');
    }
});
...
var popover = phonon.popover('#channels-menu')
  , chan = subscriptionURL.substr(1).split('/')[0] || '2230';
...
var updateChannelList = function(callback) {
    $.getJSON('?ch=' + chan + (highlightedPost ? ('&mid=' + highlightedPost) : ''), function(data) {
        popover.setList(data, function(item) {
            ...
            if (item.date)
                html += '<span class="date ' + item.date.replace(/[^dmhs]/g, '') + '" data-value="' + item.ch + '">' + htmlEscape(item.date) + '</span>';
